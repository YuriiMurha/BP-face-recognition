...
The complete codebase, datasets, trained model weights, and the rendered version of this thesis are stored on the medium attached to the thesis. The repository contains a README file at its root that mirrors the most-frequently-needed sections of this manual in a format suitable for reading without LaTeX rendering. The system manual that follows this user manual documents the internal organisation of the codebase for readers who wish to extend or modify the system.
