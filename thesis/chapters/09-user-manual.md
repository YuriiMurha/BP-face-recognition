...
The complete source code of the system is stored on the CD attached to the thesis, with a README file at its root that mirrors the most-frequently-needed sections of this manual in a format suitable for reading without LaTeX rendering. The custom datasets and the trained model weights are not on the CD: the datasets contain personal data, and the models can be regenerated by running the training pipeline; both are available from the author on request. The system manual that follows this user manual documents the internal organisation of the codebase for readers who wish to extend or modify the system.
