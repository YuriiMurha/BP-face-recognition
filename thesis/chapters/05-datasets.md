...
**Table 5.1: Data sources used in the thesis corpus.** All image counts are post-augmentation totals (see §5.4.3); raw image counts before augmentation are roughly two orders of magnitude smaller per source.
...
| **Total**       | **48**     | **9,450**        |                                                  |                                                            |
| **14-class subset** | **14** | **7,080** | webcam + seccam | Headline experiments (Chapter 7) |

The webcam subset is small in identity count but dense in samples and is a clean reference point against which degraded captures can be compared. The security camera subset reproduces the conditions the final system is intended for: oblique angles, motion blur, variable illumination, subjects unaware of the camera. LFW [7] contributes the bulk of the identity count and was used at the beginning of the project to verify that the training and evaluation pipeline reproduced the expected behaviour on a benchmark with well-understood characteristics. Once the pipeline was validated, the headline closed-set comparison in Chapter 7 was confined to the 14-identity custom subset, because that subset matches the deployment domain the thesis targets.
...
Augmentation was applied to the training split only, using the Albumentations library [9]. Each raw training image was expanded into a fixed multiplier of augmented variants through random compositions drawn from the transform families in Table 5.3. The validation and test splits were left as raw crops so that reported metrics reflect behaviour on unaugmented, in-distribution images. All image counts elsewhere in this chapter and in Chapter 7 are post-augmentation training counts unless explicitly stated otherwise.
