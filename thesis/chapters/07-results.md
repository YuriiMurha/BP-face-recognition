...
The detection benchmark evaluated four face detection methods: MediaPipe BlazeFace [CITE: Bazarevsky et al. 2019], MTCNN [CITE: Zhang et al. 2016], Haar Cascade [CITE: Viola and Jones 2001], and Dlib HOG [CITE: Dalal and Triggs 2005, King 2009]. Each method processed the same set of 19 raw surveillance camera frames drawn from the three custom sources (webcam and two security-camera placements). The frames depict subjects at varying distances (estimated 1 to 5 meters), under mixed artificial lighting, and at multiple angles including frontal, three-quarter, and profile views.

The 19 frames are deliberately a small, hand-curated *test subset* rather than the entire dataset. Detection metrics require ground-truth bounding boxes drawn by a human; the broader corpus of thousands of cropped face images in Chapter 5 cannot serve as a detection benchmark because no per-face rectangles exist for those images, only filename-encoded identity labels. For this evaluation, 19 raw frames were sampled to cover the range of capture conditions in the surveillance subset and were manually annotated in LabelMe, producing 26 ground-truth face bounding boxes across the 19 images. Each ground-truth box is a two-point rectangle in image coordinates. All frames were resized to a maximum of 800 pixels on the longest edge before processing; ground-truth boxes were rescaled by the same factor so that all intersection-over-union (IoU) computations took place in the resized image's coordinate space.

Augmenting the detection test set was not pursued because the augmentation pipeline used in this thesis (Chapter 5.4.3) transforms image pixels but does not track bounding-box coordinates alongside the pixel transformations. Augmenting the 19 frames would therefore detach the boxes from the faces they annotate, which defeats the purpose of a ground-truth benchmark. Expanding the detection test set would require either fresh manual annotation of additional raw frames or an augmentation pipeline extended to keep box coordinates in sync with geometric transforms; both are noted as future work in Chapter 8.

No GPU acceleration was used during benchmarking; each detector was run in an isolated Python subprocess with CPU-only execution forced through `CUDA_VISIBLE_DEVICES=-1` to prevent cross-detector state collisions in shared deep-learning runtimes.
...
Progressive Unfreezing wins on every verification metric: it achieves the lowest EER (0.090), the highest TAR at both fixed-FAR operating points (0.632 at FAR = 0.01, 0.307 at FAR = 0.001), and the highest AUC (0.971). Triplet Loss comes second on every metric (EER 0.179, AUC 0.910), and Transfer Learning is a distant third (EER 0.296, AUC 0.771). All verification metrics in this section are reported as decimal fractions on $[0, 1]$ to match the figure legends and the convention used throughout the biometric literature.
...
The error structure of Progressive Unfreezing is illustrated in Figure 7.6, which plots the test-set confusion matrix shaded by per-row recall. The dominant diagonal confirms the 99.15% accuracy reported above; the few off-diagonal counts correspond to the boundary samples discussed in §7.3, all of which involve classes with small per-class supports.

![Confusion matrix for the Progressive Unfreezing model on the 1,062-sample test set](../figures/confusion_matrix_pu.png)
**Figure 7.6:** Confusion matrix for the Progressive Unfreezing model on the 1,062-sample test set, shaded by per-row recall. Diagonal cells show correctly classified samples (counts overlaid); off-diagonal cells show misclassifications. The four classes with double-digit off-diagonal contributions correspond to the three under-represented identities in Table 7.5 that account for most of the residual 0.85% error budget.
