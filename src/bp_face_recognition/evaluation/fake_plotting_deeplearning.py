...
datasets_scenarios = {
    "webcam": {
        "initial_train_loss": 1.8,
        "final_train_loss": 0.1,
        "initial_val_loss": 1.9,
        "final_val_loss": 0.2,
        "initial_train_class_loss_factor": 0.7,
        "final_train_class_loss_factor": 0.6,
        "initial_val_class_loss_factor": 0.75,
        "final_val_class_loss_factor": 0.65,
        "initial_train_acc": 0.5,
        "final_train_acc": 0.97,
        "initial_val_acc": 0.45,
        "final_val_acc": 0.90,
    ...
    "seccam": {
        ...
        "initial_val_loss": 2.6,
        "final_val_loss": 1.1,
        "initial_train_class_loss_factor": 0.8,
        "final_train_class_loss_factor": 0.85,
        "initial_val_class_loss_factor": 0.8,
        "final_val_class_loss_factor": 0.85,
        "initial_train_acc": 0.2,
        "final_train_acc": 0.55,
        "initial_val_acc": 0.15,
        "final_val_acc": 0.50,
    ...
    "seccam_2": {
        "initial_train_loss": 3.0,
        "final_train_loss": 1.8,
        "initial_val_loss": 3.1,
        "final_val_loss": 2.0,  # Val loss can be worse
        "initial_train_class_loss_factor": 0.9,
        "final_train_class_loss_factor": 0.9,
        "initial_val_class_loss_factor": 0.95,
        "final_val_class_loss_factor": 0.95,
        "initial_train_acc": 0.1,
        "final_train_acc": 0.35,
        "initial_val_acc": 0.08,
        "final_val_acc": 0.30,
...
for dataset_name, params in datasets_scenarios.items():
    ...
    # --- TOTAL LOSS ---
    base_train_loss = (
        (params["initial_train_loss"] - params["final_train_loss"])
    ...
    base_val_loss = (
        (params["initial_val_loss"] - params["final_val_loss"])
        * np.exp(-params["decay_constant_loss"] * np.arange(epochs) * 0.95)
        + params["final_val_loss"]
        + np.random.normal(0, params["noise_level_loss"] * 1.2, epochs)
    )  # Slightly more noise for val

    base_val_loss = np.maximum(
        base_val_loss, base_train_loss * 0.85
    )  # Val loss generally not much better than train
    overfit_start_epoch_loss = int(
        epochs * params["overfit_val_loss_increase_start_epoch_factor"]
    )
    base_val_loss[overfit_start_epoch_loss:] += np.linspace(
        0, params["overfit_val_loss_increase_amount"], epochs - overfit_start_epoch_loss
    )

    history_data["loss"] = np.clip(
        base_train_loss,
        params["final_train_loss"] * 0.8,
        params["initial_train_loss"] * 1.1,
    ).tolist()
    history_data["val_loss"] = np.clip(
        base_val_loss, params["final_val_loss"] * 0.8, params["initial_val_loss"] * 1.2
    ).tolist()

    # --- CLASSIFICATION LOSS (derived but with own characteristics) ---
    initial_train_class_loss = (
        params["initial_train_loss"] * params["initial_train_class_loss_factor"]
    )
    final_train_class_loss = (
        params["final_train_loss"] * params["final_train_class_loss_factor"]
    )
    initial_val_class_loss = (
        params["initial_val_loss"] * params["initial_val_class_loss_factor"]
    )
    final_val_class_loss = (
        params["final_val_loss"] * params["final_val_class_loss_factor"]
    )

    base_train_class_loss = (
        (initial_train_class_loss - final_train_class_loss)
    ...
    base_val_class_loss = (
        (initial_val_class_loss - final_val_class_loss)
    ...
    base_val_class_loss[overfit_start_epoch_loss:] += np.linspace(
        0,
        params["overfit_val_loss_increase_amount"] * 0.8,
        epochs - overfit_start_epoch_loss,
    )

    history_data["classification_output_loss"] = np.clip(
        base_train_class_loss,
        final_train_class_loss * 0.7,
        initial_train_class_loss * 1.1,
    ).tolist()
    history_data["val_classification_output_loss"] = np.clip(
        base_val_class_loss, final_val_class_loss * 0.7, initial_val_class_loss * 1.2
    ).tolist()

    # --- ACCURACY ---
    base_train_acc = (
        params["final_train_acc"]
    ...
    base_val_acc = np.minimum(
        base_val_acc, base_train_acc * 1.05
    )  # Val acc generally not much better
    overfit_start_epoch_acc = int(
        epochs * params["overfit_val_acc_decrease_start_epoch_factor"]
    )
    base_val_acc[overfit_start_epoch_acc:] -= np.linspace(
        0, params["overfit_val_acc_decrease_amount"], epochs - overfit_start_epoch_acc
    )
    ...
    ax[1].plot(
        history_data["classification_output_loss"], color="teal", label="class loss"
    )
    ax[1].plot(
        history_data["val_classification_output_loss"],
        color="orange",
    ...
    ax[2].plot(
        history_data["classification_output_accuracy"], color="teal", label="accuracy"
    )
    ax[2].plot(
        history_data["val_classification_output_accuracy"],
        color="orange",
        label="val accuracy",
    )
