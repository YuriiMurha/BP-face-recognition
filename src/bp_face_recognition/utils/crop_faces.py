...
IMAGE_SIZES = {"webcam": (480, 640), "seccam": (1280, 800), "seccam_2": (1280, 800)}
...
def process_datasets():
    ...
    for dataset_source in tqdm(DATASET_SOURCES, desc="Processing Sources"):
        for dataset_type in tqdm(
            DATASET_TYPES, desc=f"Subsets for {dataset_source}", leave=False
        ):
            ...
            for img_path in tqdm(
                image_files,
                desc=f"Cropping {dataset_source}/{dataset_type}",
                unit="img",
                leave=False,
            ):
                label_path = "Unknown"
                try:
                    ...
                    for label, bbox in zip(labels, bboxes):
                        ...
                        if xmax <= xmin or ymax <= ymin:
                            print(
                                f"Warning: Invalid bounding box {bbox} for image {img_path}. Skipping."
                            )
                            continue
                        ...
                        if (
                            cropped_face.size == 0
                            or cropped_face.shape[0] < 1
                            or cropped_face.shape[1] < 1
                        ):
                            print(
                                f"Warning: Empty or invalid crop from {img_path} for bbox {bbox}. Skipping."
                            )
                            continue

                        # Resize cropped face to the target size
                        cropped_face_resized = cv2.resize(
                            cropped_face, (IMG_WIDTH, IMG_HEIGHT)
                        )

                        # Create output directories for images and labels
                        output_dir = os.path.join(
                            OUTPUT_CROPPED_FACES_DIR, dataset_source, dataset_type
                        )
                    ...
                    print(
                        f"Error processing {img_path} with label {label_path}: {e}. Skipping."
                    )

    print(
        f"Dataset preparation complete. Total cropped faces saved: {processed_faces_count}"
    )
